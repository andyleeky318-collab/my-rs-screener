...
with st.sidebar:
    ...
    st.markdown("---")
    screenshot_clicked = st.button("📸 Screenshot → Telegram", use_container_width=True)
...
if screenshot_clicked:
    ...
    if not tg_token or not tg_chat:
        st.sidebar.warning("Telegram secrets missing — cannot send screenshot.")
    else:
        capture_html = f"""
        <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
        <div id="ss-status" style="font-family:sans-serif;color:#ccc;padding:6px;font-size:13px;">📸 Capturing page…</div>
        <script>
        (async function() {{
            ...
            try {{
                ...
                const scale = fullCanvas.width / doc.documentElement.scrollWidth;
                const cropX = Math.round(SIDEBAR_WIDTH * scale);
                const croppedWidth = fullCanvas.width - cropX;
                const croppedCanvas = document.createElement('canvas');
                croppedCanvas.width = croppedWidth;
                croppedCanvas.height = fullCanvas.height;
                const ctx = croppedCanvas.getContext('2d');
                ctx.drawImage(fullCanvas, cropX, 0, croppedWidth, fullCanvas.height, 0, 0, croppedWidth, fullCanvas.height);

                statusEl.innerText = '📄 Building PDF…';

                const {{ jsPDF }} = window.jspdf;
                const pageHeightPx = Math.round(croppedWidth * 1.4142); // A4 portrait ratio
                const totalPages = Math.max(1, Math.ceil(croppedCanvas.height / pageHeightPx));
                let pdf = null;

                for (let i = 0; i < totalPages; i++) {{
                    const sliceHeight = Math.min(pageHeightPx, croppedCanvas.height - i * pageHeightPx);
                    const sliceCanvas = document.createElement('canvas');
                    sliceCanvas.width = croppedWidth;
                    sliceCanvas.height = sliceHeight;
                    const sctx = sliceCanvas.getContext('2d');
                    sctx.drawImage(croppedCanvas, 0, i * pageHeightPx, croppedWidth, sliceHeight, 0, 0, croppedWidth, sliceHeight);
                    const imgData = sliceCanvas.toDataURL('image/jpeg', 0.85);

                    if (i === 0) {{
                        pdf = new jsPDF({{ orientation: 'p', unit: 'px', format: [croppedWidth, sliceHeight] }});
                    }} else {{
                        pdf.addPage([croppedWidth, sliceHeight]);
                    }}
                    pdf.addImage(imgData, 'JPEG', 0, 0, croppedWidth, sliceHeight);
                }}

                const pdfBlob = pdf.output('blob');

                statusEl.innerText = '📤 Sending to Telegram…';

                const formData = new FormData();
                formData.append('chat_id', '{tg_chat}');
                formData.append('document', pdfBlob, 'dashboard_screenshot.pdf');
                formData.append('caption', 'Dashboard Screenshot');

                const resp = await fetch('https://api.telegram.org/bot{tg_token}/sendDocument', {{
                    method: 'POST',
                    body: formData,
                }});
                const result = await resp.json();
                statusEl.innerText = result.ok ? '✅ Sent to Telegram!' : ('❌ Telegram error: ' + JSON.stringify(result));
            }} catch (err) {{
