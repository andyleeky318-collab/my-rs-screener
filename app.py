...
@st.cache_data(ttl=3600)
def get_lev_etf_volumes(tickers):
    data = yf.download(tickers, period="1mo", interval="1d", progress=False, auto_adjust=False)
    # Check if 'Volume' exists to avoid KeyError if yfinance fails
    if 'Volume' in data.columns:
        avg_vols = data['Volume'].mean().to_dict()
        return {k: round(v / 1e6, 2) for k, v in avg_vols.items() if not pd.isna(v)}
    return {}

# ──────────────────────────────────────────────
# LEVERAGED ETF BUBBLE CHART
# ──────────────────────────────────────────────
st.markdown("---")
...
LEVERAGED_ETFS = [
    ...
    {"t":"AVGU","lev":2,"vol":1.8,"cat":"broad","under":"AVGO","desc":"2x Broadcom"},
...
# 1. Gather all ticker symbols from your structural array
ticker_list = [etf["t"] for etf in LEVERAGED_ETFS]

# 2. Fetch the live 1-month average volume data map via yfinance
live_volumes = get_lev_etf_volumes(ticker_list)

# 3. Dynamically overwrite the default 'vol' values with the fresh data
for etf in LEVERAGED_ETFS:
    symbol = etf["t"]
    if symbol in live_volumes:
        etf["vol"] = live_volumes[symbol]

# 4. Serialize the refreshed list into the proper JSON layout
etfs_json = json.dumps(LEVERAGED_ETFS)

# 5. Build your raw D3.js or HTML5 canvas bubble map script inside the component string container
bubble_html = f"""
<!DOCTYPE html>
<html>
<head>
    <script src="https://d3js.org/v6.min.js"></script>
    <style>
        body {{ margin: 0; background-color: #0e1117; font-family: sans-serif; color: white; overflow: hidden; }}
        .bubble {{ stroke: #1e222b; stroke-width: 1.5px; opacity: 0.85; transition: opacity 0.2s; }}
        .bubble:hover {{ opacity: 1; stroke: #fff; }}
        text {{ font-size: 11px; font-weight: bold; fill: #ffffff; text-anchor: middle; pointer-events: none; }}
        .subtitle {{ font-size: 8px; fill: #a3a8b4; font-weight: normal; }}
    </style>
</head>
<body>
    <div id="chart"></div>
    <script>
        ...
        // Map categories to distinctive visual indicators
        const colorScale = d3.scaleOrdinal()
            .domain(["bull", "bear", "sector", "broad"])
            .range(["#22c55e", "#ef4444", "#3b82f6", "#a855f7"]);

        // Establish a non-linear scale logic for uniform rendering layout sizing
        const radiusScale = d3.scaleSqrt()
            .domain([0, d3.max(data, d => d.vol)])
            .range([15, 65]);

        // Run D3 force layout configurations to prevent node overlaps
        const simulation = d3.forceSimulation(data)
            .force("x", d3.forceX(width / 2).strength(0.05))
            .force("y", d3.forceY(height / 2).strength(0.05))
            .force("collide", d3.forceCollide(d => radiusScale(d.vol) + 3))
            .on("tick", ticked);
        ...
        node.append("circle")
            .attr("class", "bubble")
            .attr("r", d => radiusScale(d.vol))
            .attr("fill", d => colorScale(d.cat))
            .append("title")
            .text(d => `${{d.desc}}\\nAvg Volume: ${{d.vol}}M`);

        // Overlay ticker labels
        node.append("text")
            .attr("dy", "-2")
            .text(d => d.t);

        // Overlay volume data text strings
        node.append("text")
            .attr("class", "subtitle")
            .attr("dy", "10")
            .text(d => `${{d.vol}}M`);
        ...
        function ticked() {{
            node.attr("transform", d => `translate(${{Math.max(65, Math.min(width - 65, d.x))}}, ${{Math.max(65, Math.min(height - 65, d.y))}})`);
        }}
    </script>
</body>
</html>
