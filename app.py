...
def setup_badge(sym, is_new=False, is_removed=False, extra_prefix="", extra_suffix="", extra_suffix_color="#888888", extra_style="", extra_text_color=None):
    ...
    if sym in cloud_valid_syms:
        return (f'<div class="ticker-badge" style="background-color:#378ADD; border:1px solid #378ADD;{extra_style}">{extra_prefix}'
                f'<span style="color:#111111;font-weight:bold;">{sym}</span>{suffix_html}</div>')
    if is_new:
...
if all_data:
    ...
    for row_num, (i, row) in enumerate(df_main.iterrows(), start=1):
        ...
        if trend_html == "🟢🟢🟢🟢":
            trend_html = f'<span style="border: 2px solid #32CD32; color: inherit; font-weight: bold; padding: 1px 6px; border-radius: 3px; display: inline-block;">{trend_html}</span>'
...
# ==============================================================================
# 24. STAGE % BY INDUSTRY GROUP — for a fixed watchlist of tickers
# ==============================================================================
#st.markdown("---")
...
stage_pct_df_t = stage_pct_df.set_index("Ticker").T.rename(index={"Stage1 %": "stg1 %", "Stage2 %": "stg2 %", "Stage3 %": "stg3 %", "Stage4 %": "stg4 %"})
...
def render_market_regime_gauge(value, title="Market Regime"):
    ...
    _PIVOT_X_FRAC = 0.5
    _PIVOT_Y_FRAC = 0.08
    _NEEDLE_LEN_PX = 190   # tune this until the tip touches the donut ring
    _HUB_R_PX = 12
...
if gapper_list or gapper_yest:
    ...
    for sym in gapper_list:
        ...
        quality_style = "background:#378ADD;" if sym in gapper_quality_pass else ""  # NEW
...
if analyst_grades_df.empty:
    ...
    for sym, row in top_movers.iterrows():
        ...
        html_grades += (
            f'<div class="ticker-badge"><span class="ticker-name">{sym}</span>'
            f'<span class="ticker-rs" style="color:{color};font-weight:bold;margin-left:5px;">▲{ups} ▼{downs}</span></div>'
        )
    html_grades += "</div>"
...
if not rrg_data_full:
    ...
    with rrg_col_a:
        rrg_selected = st.multiselect(
            "Tickers on RRG",
            options=all_rrg_tickers_sorted,
    ...
    rrg_data = (
        ...
        if rrg_selected else rrg_data_full
    )
    ...
    if not rrg_data:
        ...
        x_pad = max((x_max - x_min) * 0.15, 0.5)
        ...
        for i, ticker in enumerate(sorted(plot_data.keys())):
            ...
            # Start marker (hollow circle) — where it was
            fig_rrg.add_trace(go.Scatter(
                x=[xs[0]], y=[ys[0]], mode="markers",
                marker=dict(size=9, color="rgba(0,0,0,0)", line=dict(color=color, width=1.8)),
                showlegend=False, hoverinfo="skip",
            ))

            # End marker (star) + ticker label — where it is now
            fig_rrg.add_trace(go.Scatter(
...
def compute_market_verdict():
    ...
    accumulation_rows_v = _safe("accumulation_rows", [])
    valid_accum_vals = [r["Avg Value"] for r in accumulation_rows_v if r.get("Avg Value") is not None]
    if valid_accum_vals:
...
# Fill in the Lazy Charts title now that every chart above has run — red with
...
#     # ── 28. Pine RS Table — fresh short-term acceleration (RS5 leading RS63/126)
#     pine_rs_rows_v = _rrd_get("pine_rs_rows", [])
#     pine_fresh_accel = []   # RS5 already hot but longer windows haven't caught up yet = early rotation
#     pine_52w_high = []      # currently at/above 52-week close per the table's own flag

#     for r in pine_rs_rows_v:
#         try:
#             rs5 = r.get("RS5")
#             rs63 = r.get("RS63")
#             rs126 = r.get("RS126")
...
#             if sym is None:
#                 continue
#             if rs5 is not None and not pd.isna(rs5) and rs5 >= 80 and (
#                 (rs63 is not None and not pd.isna(rs63) and rs63 < 80) or
#                 (rs126 is not None and not pd.isna(rs126) and rs126 < 80)
#             ):
#                 pine_fresh_accel.append(sym)
#             if r.get("Is52WHigh"):
#                 pine_52w_high.append(sym)
#         except Exception:
#             continue

#     pine_score = (
#         _rrd_scale(len(pine_fresh_accel), 3) * 0.6
#         + _rrd_scale(len(pine_52w_high), 3) * 0.4
#     )
#     add_pillar(
#         "Pine RS Table — Fresh Acceleration", pine_score, 0.04,
#         f"{len(pine_fresh_accel)} tickers RS5≥80 but RS63/126 lagging (early rotation) | "
#         f"{len(pine_52w_high)} at 52W-high close",
#         sorted(set(pine_fresh_accel) | set(pine_52w_high))
#     )
...
#     "Unusual Volume Cluster (HVE/HVQ/HVM)",
#     "Pine RS Table — Fresh Acceleration",   # NEW
# ]
